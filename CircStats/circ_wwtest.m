function [pval, table, message,rw] = circ_wwtest(varargin)
...
message = checkAssumption(rw,mean(pn));
...
function message = checkAssumption(rw,n)
  message = '';
  if n >= 11 && rw<.45
    message = 'Test not applicable. Average resultant vector length < 0.45.';
    warning(message) %#ok<WNTAG>
  elseif n<11 && n>=7 && rw<.5
    message = 'Test not applicable. Average number of samples per population 6 < x < 11 and average resultant vector length < 0.5.';
    warning(message) %#ok<WNTAG>
  elseif n>=5 && n<7 && rw<.55
    message = 'Test not applicable. Average number of samples per population 4 < x < 7 and average resultant vector length < 0.55.';
    warning(message) %#ok<WNTAG>
  elseif n < 5
    message = 'Test not applicable. Average number of samples per population < 5.';
    warning(message) %#ok<WNTAG>
  end
