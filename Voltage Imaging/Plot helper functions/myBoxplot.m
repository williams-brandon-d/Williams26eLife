...
if ~isempty(data_all_mat)
    ...
    for j=1:length(h1)
        patch(get(h1(j),'XData'),get(h1(j),'YData'),colors(j,:),'FaceAlpha',alphas(1));
        line(h2(j).XData,h2(j).YData,'Color','k','Linewidth',1)
    end
    drawnow;
%     set(findobj(gcf, 'type', 'line', 'Tag', 'Median'), 'Color', 'k');    
%     BoxPlotFormat(gca,'all', 'Median','Color','k');
end
...
if isfield(stats,'results')
    ...
    plotSignificance(stats,xPos,xTickLabels,dataMax,y.scale)
end
