...
% gamma only
% calculate VS and phase for each neuron, for each cell type
% save excel file 

maxNeurons = max(cellfun(@numel,gammaCell)); % max number of neurons for all cell types

VSneuron = NaN(nCellTypes,maxNeurons);
MUneuron = VSneuron;
Zneuron = VSneuron;

% plot resultant vector for pooled gamma phase for each cell type
VS = NaN(nCellTypes,1);
mu = VS;
Z = VS;

gammaPhases = cell(nCellTypes,1);
...
for iCell = 1:nCellTypes
    ...
    gammaPhases{iCell} = phases_mat; % save combined phases for each cell type 

    % calculate pooled vector strength and angle
    [VS(iCell), mu(iCell), Z(iCell)] = vector_strength(phases_mat);

    [color,~] = getColorAlpha(cell_types(iCell),1);
    colors(iCell,:) = color{1};    

    % calculate individual vector strength and angle
    nNeurons = numel(phaseCell);
    for iNeuron = 1:nNeurons
        phaseNeuron = phaseCell{iNeuron}; % vector of phases
        if numel(phaseNeuron) >= 10
            [VSneuron(iCell,iNeuron), MUneuron(iCell,iNeuron), Zneuron(iCell,iNeuron)] = vector_strength(phaseNeuron);
        end
    end

end

% individual pyramidal neurons exibit bimodal distribution of resultant phases
group1 = [-2.2935096, -1.742745425, -1.079564039, -1.193246982, -1.699811258]; 
group2 = [1.345973216, 2.089145946, 2.711182679, 2.492333704, 2.499254237, 1.827213252,...
          0.808363946, 1.718245131, 1.449341899, 1.306581566, 0.748129075, 1.876745434,...
          0.890061022 ];

% bimodal stats
[mu1, ul1, ll1] = circ_mean(group1');
[mu2, ul2, ll2] = circ_mean(group2');

% compare separate pyramidal groups to stellate and FS cells


% plot VS and phase for each neuron per cell type
MuData = reshape(MUneuron',[],1);
VSdata = reshape(VSneuron',[],1);
N = numel(MuData);
colorData = zeros(N,3);
for iCell = 1:nCellTypes
    colorData((maxNeurons*(iCell-1)+1):(maxNeurons*iCell),:) = repmat(colors(iCell,:),maxNeurons,1);
end
figScatter = figure;
polarscatter(MuData,VSdata,[],colorData,'filled');
ax = gca;
...
if savenum
    saveFilename = [saveFolder filesep sprintf('%s Gamma Vector Strength Individual',dataSet)];
    print(figScatter,'-vector','-dsvg',saveFilename);
end
...
% stats comparison for neuron averages
% stat tables for VS and MU
% summary stats and comparisons
allVS = cell(nCellTypes,1);
allMU = allVS;

for iCell = 1:nCellTypes
    allVS{iCell} = VSneuron(iCell,:);
    allMU{iCell} = MUneuron(iCell,:);
end

saveFilename = [saveFolder filesep sprintf('%s Gamma Vector Strength stats.xlsx',dataSet)];
statsVS = myMultipleIndependentGroupStats(allVS,cell_types,'','linear',saveFilename,savenum);

% circular stats
saveFilename = [saveFolder filesep sprintf('%s Gamma MU circ stats.xlsx',dataSet)];
statsMU = myMultipleIndependentCircStats(allMU,cell_types,'','linear',saveFilename,savenum);

% circ stats with split pyramidal groups
newMU = allMU(~strcmp(cell_types,'pyramidal')); % remove pyramidal cell group
pyramidalMU = allMU(strcmp(cell_types,'pyramidal'));
pyramidalMU = pyramidalMU{1};
newMU{end+1} = pyramidalMU(pyramidalMU < 0);
newMU{end+1} = pyramidalMU(pyramidalMU >= 0);
new_cell_types = {'stellate','fast spiking','pyramidal1','pyramidal2'};
saveFilename = [saveFolder filesep sprintf('%s Gamma MU circ pyramidal split stats.xlsx',dataSet)];
myMultipleIndependentCircStats(newMU,new_cell_types,'','linear',saveFilename,savenum);

% axial stats
% shift allMU data by 180 degrees for axial stats
% axial stats do not capture the mean correctly
for iCell = 1:nCellTypes
    allMU{iCell,1} = mod(allMU{iCell,1} + pi + pi, 2*pi) - pi;
end
saveFilename = [saveFolder filesep sprintf('%s Gamma MU axial stats.xlsx',dataSet)];
statsMU = myMultipleIndependentGroupStats(allMU,cell_types,'','linear',saveFilename,savenum);
...
% stats for pooled spike-phase data across cell types
saveFilename = [saveFolder filesep sprintf('%s Gamma MU combined stats.xlsx',dataSet)];
statsMUall = myMultipleIndependentCircStats(gammaPhases,cell_types,'','linear',saveFilename,savenum);


if savenum
    saveFilename = [saveFolder filesep sprintf('%s Gamma Vector Strength Combined',dataSet)];
